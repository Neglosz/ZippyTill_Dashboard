...
/**
 * Validates that all items in the order have valid (non-expired) batches
 * @private
 */
const validateItemsExpiry = async (items, today) => {
...
/**
 * Creates a map of product_id -> promotion_id for active promotions
 * @private
 */
const getActivePromotionsMap = async (branchId, today) => {
  ...
  if (activePromos) {
    activePromos.forEach((promo) => {
      if (promo.promotion_items) {
        promo.promotion_items.forEach((item) => {
          promoLookup[item.product_id] = promo.id;
        });
      }
    });
  }
...
/**
 * Handles stock decrement and sends notifications if stock is low
 * @private
 */
const updateStockAndNotify = async (item, branchId) => {
  try {
    ...
    if (rpcError) {
      ...
      if (productData) {
        await supabase
          .from("products")
          .update({ stock_qty: Math.max(0, productData.stock_qty - item.qty) })
          .eq("id", item.productId)
          .eq("store_id", branchId);
      }
    }
...
const orderService = {
  /**
   * Orchestrates the creation of an order including stock updates and ledger entries
   */
  async createOrder(orderData, items, branchId) {
    ...
    // 1. Safety Check: Block selling expired products
    await validateItemsExpiry(items, today);

    // 2. Promotion Lookup
    const promoLookup = await getActivePromotionsMap(branchId, today);
    ...
    // 5. Asynchronously process Stock Updates and Notifications
    const stockUpdatePromises = items.map(item => updateStockAndNotify(item, branchId));
    await Promise.all(stockUpdatePromises);

    // 6. Record in Ledger (account_transactions)
    const isCreditSalePending = orderData.paymentStatus === "pending" && orderData.paymentMethod === "credit_sale";
    if (!isCreditSalePending) {
      try {
  ...
  /**
   * Fetches recent orders with support for date-based filtering
   */
